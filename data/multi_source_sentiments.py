...
class SentimentGetter:
    def __init__(self, selectors = {'guardian' : {'selector': 'guardian_*.csv'}, 'mirror' : {'selector': 'mirror_*.csv'}, 'telegraph': {'selector':'telegraph_*.csv'}, 
    'sun' : {'selector': 'sun_*.csv'}, 'mail' : {'selector': 'mail_*.csv', 'cols': ['headline', 'date', 'url'], 'topics_to_remove': ['wires','femail', 'sport', 'showbiz']}, 
    'metro' : {'selector': 'metro_*.csv'}, 'express' : {'selector': 'express_*.csv'}, 'all': {'selector': '*.csv'}}):
...
if __name__ == "__main__":
    sentiment_getter = SentimentGetter(selectors = {'mail' : 'mail_*.csv', 'all': '*.csv', })
