...
function NewspaperPage({name, topic_intro, topic_plot, time_plot, polarity_time, polarity_ratio, subjectivity_box, subjectivity_over_time, polarity_comments, subjectivity_comments}) {
  // setting shortname - name without leading 'the' if applicable
  ...
  const handleTitles = useCallback(() => {
    ...
    // using regex to split into certain number of words depending on screen width then rejoin with line break
    if ( width <= 600 ) {
    ...
    // creating titles object where keys will be used to retrieve the titles within the web app
    let titlesObj = {};
  ...
  return (
    ...
    <div className='newspaper-container page-content'>
      ...
      <TextWindow title={`${shortName} - Topics`} textArray={[topic_intro]} pageTitle={false} key={`${name}TopicsIntro`}/>
      ...
      <div ref={ref} className="graph-container" key={`${name}TopicGraph`}>
        {/* using spread operator so adding to layout with responsive height & width in one line, keeping it as single object */}
